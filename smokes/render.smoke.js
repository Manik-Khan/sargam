...
export const smokes = [
  ...
  },
  {
    ...
    fn: () => {
      ...
      assert.equal(row.querySelector('.sr-line-repeat-open').style.gridColumn, '1');
      assert.equal(sthayiCells(root)[0].style.gridColumn, '2');
      assert.equal(
        Number(row.querySelector('.sr-line-repeat-close').style.gridColumn)
          > Number(sthayiCells(root).at(-1).style.gridColumn),
        true
      );
    },
  },
  ...
  {
    name: 'render: a cross-matra di-ri is one continuous V aligned to both attacks',
    fn: () => {
      const { doc, problems } = parseDocument('tal: tintal\n\nS R\n> di-ri\n');
      assert.deepEqual(problems, []);
      const root = renderDocument(doc);
      ...
      assert.deepEqual(
        [...root.querySelectorAll('[data-bol-attack-ordinal]')].map((slot) => slot.textContent),
        ['', '']
      );
    },
  },
