...
- `sa:` — playback pitch of Sa, the key for playback, the CDE display, and MusicXML. **Default `C` — the sarod's key** *(ruled by M 2026-07-16; was C#)*. In the New Document form as "Key (Sa)" with M's three anchors: C sarod · D sitar · A vocal. *(Also 2026-07-16: accepts an optional octave — `C`, `C#`, `A3`, `Bb2`; a bare letter sits at octave 3, landing the anchors where the instruments live (M): sarod `C` → C3 ≈ 131 Hz, sitar `D` → D3, vocal classes `A` → A3 = 220 Hz — A3 chosen in M's classes to sit between male and female ranges. The default remains C# pending M's ruling on changing it.)*
...
**Live staff-notation toggle** *(M's request 2026-07-16; the EXPORT half shipped as `western.js` + the "Staff ↗" button)*: rendering staves live in-app needs VexFlow plus tuplet/beam/ledger layout — its own milestone. The musical core is done and smoked: scale-degree-preserving spelling (each sargam degree owns a letter; komal/tivra are the accidental — Sa=C → komal ga = E♭, Sa=D → the same is F♮), plus MusicXML with tal-derived meter, barline ties, acciaccatura kans. Next increments, in order of value: `<time-modification>` tuplet markup; then VexFlow for the live toggle.
