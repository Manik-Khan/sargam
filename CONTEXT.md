...
**Wave B state:** `src/shell/audio.js` SHIPPED — `createPlayer(env)`: injected context/timers (smoked with a fake clock), lookahead pump (25ms/100ms), synchronous `play()` on gesture, pause/resume position math, `setLoop({from,to})` wrap, per-track gain/mute (melody/tick), `onCursor`/`onEnded`. Voices are STARTING POINTS for Wave C (triangle pluck; glideFrom → setTargetAtTime ramp; tick accents sam 1200sq / khali 420sin / vibhag 880sq / plain 660sin). **Transport UI: BUILT (mock approved with M's redline).** `Transport.jsx` + App wiring + `makeAudioEnv` + `setDirective` (files.js, smoked). REDLINE APPLIED: the BPM field reads and WRITES `tempo:` — M revised the earlier provenance ruling: `laya:` carries the tradition's speed word (full spectrum ati vilambit→ati drut, exact ladder PENDING M — proposed 7 rungs in spec §3.1 are Claude's inference, confirm before relying); `tempo:` is the literal playback number. Loop off/line/section scoped to the text cursor; Space play/pause (guarded from fields); doc edits stop playback and reload the schedule; cursor highlight via opts.activeCursor. (Superseded mock note removed — see redline above.) After approval: wire App.jsx (player + transport + cursor highlight via `opts.activeCursor` — render seam exists, cursor events carry section/line/matra indices). Then Wave C ear pass — judged by M, never "good enough for verification."
