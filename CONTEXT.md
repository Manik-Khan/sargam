...
- Grid users can persist either **Cells** (only written matras are boxed) or **Graph Paper** (one uninterrupted ruled writing surface, with transparent notation placed directly in its squares) across both the rendered score and Grid Write. PDF export separately offers Clean, Matra cells, and the same true full-sheet Graph paper. Dense rhythmic clusters, kan/approach ornaments, and repeat endings reserve two or three grid units rather than becoming microscopic; ordinary matras stay compact. Tala markers remain inset in their upper-left coordinate lane instead of following delayed attacks into vibhag dividers.
