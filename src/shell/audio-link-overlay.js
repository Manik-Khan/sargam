...
export function mountAudioLinkOverlays(root, links = [], options = {}) {
  ...
  for (const link of links) {
    ...
    const label = `${link.recording?.name || 'Sargam Music recording'} · ${formatTime(link.startTime)}–${formatTime(link.endTime)}`;
    ...
    badge.setAttribute('aria-label', `Load linked Sargam Music loop ${formatTime(link.startTime)} to ${formatTime(link.endTime)}`);
    badge.title = label;
    badge.textContent = '♪';
